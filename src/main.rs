                ...
                match num.parse::<u32>() {
                    Ok(x) => {
                        if x > 0 && x <= 40 {
                            ...
                            println!("Error: minimum version must be 1 to 40 inclusive but given '{}'.", num);
                            exit(0);
                        }
                    },
                    Err(_) => {
                        println!("Error: minimum version must be integer but given '{}'.", num);
                        exit(0);
                    }
                }
		...
		if args[1] == "-h" {
			...
			let size = if args.len() > idx + 2 {
				match args[idx + 2].parse::<u32>() {
                    Ok(x) => x,
                    Err(_) => {
                        println!("Error: pixel size must be integer but given '{}'.", args[idx + 2]);
                        exit(0);
                    }
                }
			} else {
